...
from model_src.ofa.utils.my_dataloader import MyRandomResizedCrop, MyDistributedSampler
...
class ImagenetDataProvider(DataProvider):
	...
	def __init__(self, save_path=None, train_batch_size=256, test_batch_size=512, valid_size=None, n_worker=32,
		...
		if not isinstance(self.image_size, int):
			from model_src.ofa.utils.my_dataloader import MyDataLoader
		...
		if valid_size is not None:
			...
			if num_replicas is not None:
				...
				self.train = train_loader_class(
					train_dataset, batch_size=train_batch_size, shuffle=True, num_workers=n_worker, pin_memory=False,
				)
			self.valid = None
		...
		if num_replicas is not None:
			...
			self.test = torch.utils.data.DataLoader(
				test_dataset, batch_size=test_batch_size, sampler=test_sampler, num_workers=n_worker, pin_memory=False,
			)
		else:
			self.test = torch.utils.data.DataLoader(
				test_dataset, batch_size=test_batch_size, shuffle=True, num_workers=n_worker, pin_memory=False,
			)
